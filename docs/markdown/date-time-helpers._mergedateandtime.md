...
```typescript
export declare function _mergeDateAndTime(date: string | number | ImmutableObject<Date> | undefined, time: Immutable<DateInput>): Immutable<Date>;
```
...
|  --- | --- | --- |
|  date | string \| number \| ImmutableObject&lt;Date&gt; \| undefined | A date input specifying the date of the result. |

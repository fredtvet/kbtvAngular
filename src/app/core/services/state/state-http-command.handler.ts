...
import { StateCurrentUser } from './interfaces/global-state.interfaces';
import { StateHttpCommand } from './state-http-converter/state-http-command.interface';
import { IStateHttpCommandHandler } from './interfaces/state-http-command-handler.interface';

export interface State extends StateCurrentUser { requestQueue: Request[]; };
export interface Request { command: StateHttpCommand<any>, state: any };

...
export class AppComponent {
  ...
  constructor(
    ...
    private appConfigService: AppConfigurationService,){
      ...
      if(!this.authService.hasJwtExpired()) this.dataSyncService.syncAll();
      ...
      const continousSync$ = combineLatest(interval(1000*60*3), this.appConfigService.config$).pipe(
        tap(x => {if(!this.authService.hasJwtExpired()) this.dataSyncService.syncIfTimePassed(x[1].syncRefreshTime)})
      );

...
export class TimesheetFormViewComponent implements OnInit {
  ...
  initalizeForm(){
    this.timesheetForm = this._formBuilder.group({
      ...
      date: [{value: this.datePreset || new Date(this.timesheet.startTime), disabled: this.datePreset}, [
        Validators.required
      ]],
      timeRange: [[new Date(this.timesheet.startTime), new Date(this.timesheet.endTime)], [
        Validators.required,

...
export const SetSyncResponseReducer = _createReducer(
    ...
    (state: Immutable<StoreState>, action: Immutable<SyncStateSuccessAction>): unknown => {
        ...
        for(const prop in action.response.arrays){
            ...
            if(deletedEntities?.length)
                newState[prop] = 
                    _removeRangeByIdentifier<unknown>(state[prop], deletedEntities, propCfg.identifier);

            if(entities?.length)
                newState[prop] = 
                    _addOrUpdateRange<unknown>(newState[prop] || state[prop], entities, propCfg.identifier); 
        }
